MAKEFLAGS  := -j 1
INS         = source/beamerthememacquarie.ins
...
DOC_SRC     = doc/macquarietheme.dtx
DOC_PDF     = doc/macquarietheme.pdf

CTAN_CONTENT = README.md $(INS) $(PACKAGE_SRC) $(DOC_SRC) $(DOC_PDF) $(DEMO_SRC) $(DEMO_PDF)

DESTDIR     ?= $(shell kpsewhich -var-value=TEXMFHOME)
INSTALL_DIR  = $(DESTDIR)/tex/latex/macquarie
DOC_DIR      = $(DESTDIR)/doc/latex/macquarie
...
ctan: $(CTAN_CONTENT) ctan-version
	@tar --transform "s@\(.*\)@macquarie/\1@" -cf macquarie-$(shell date "+%Y-%m-%d").tar.gz $(CTAN_CONTENT)
